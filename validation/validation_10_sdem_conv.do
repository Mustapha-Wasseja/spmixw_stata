*! Validation 10: SDEM Convex Panel
*!
*! DGP: u = (I - lam*Wc)^{-1} eps; y = X*beta + sum_m W_m*X*theta_m + FE + u
*! lam = 0.5, beta = (1, 1), theta_1 = (-0.5, -0.5), theta_2 = (-0.5, -0.5),
*! gamma = (0.4, 0.6), sige = 0.5
version 13.0
...
mata:
    ...
    function build_banded4(real scalar N, real scalar b) {
        real matrix    W
        real colvector rs
        real scalar    i, k
    ...
    W1 = build_banded4(N, 4)
    W2 = build_banded4(N, 6)
...
print_header Validation 10: SDEM Conv Panel - M=2, true gamma=(0.4, 0.6)
...
check_param, name("lambda") estimate(`=e(rho)') truth(0.5) tol(0.20)
local NPASS = `NPASS' + r(pass)
local NFAIL = `NFAIL' + (1 - r(pass))
...
di as txt "  Validation 10 Summary: `NPASS' passed, `NFAIL' failed"
di as txt "{hline 70}"
if `NFAIL' > 0 exit 9
