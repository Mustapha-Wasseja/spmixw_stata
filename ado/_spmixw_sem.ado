*! version 0.1.0  2026-05-08  spmixw — SEM panel subroutine
*! Called by spmixw.ado; not intended for direct use.
*!
*! The spatial-error parameter λ enters the table as the row labelled `We`
*! (parallel to `Wy` in SAR). We continue to call it `rho` internally so
*! the same `e(rho)` accessor works across SAR and SEM.

program _spmixw_sem, eclass
    version 13.0
    ...
    local nobs = r(N)
    if `nobs' != `N' * `TT' {
        di as err "spmixw v0.2.1 requires a balanced panel (got N=`N', T=`TT', " ///
    ...
    // -- Display -------------------------------------------------------------
    if "`nodisp'" == "" {
        _spmixw_sem_display, level(`level')
    }
end


// ----------------------------------------------------------------------
// Mata bridge — pulls Stata data, builds detval, calls _spmixw_sem
// ----------------------------------------------------------------------
version 13.0
mata:
mata set matastrict on

void _spmixw_sem_caller(string scalar  yvar,
                        string scalar  xvars,
                        real   scalar  N,
                        real   scalar  TT,
                        ...
                        string scalar  rho_pm_scalar)
{
    ...
    // Per-unit log-determinant grid, then panel-scale by T.
    detval = _spmixw_logdet_exact(W, rmin, rmax, gridstep)
    detval_panel = detval
    detval_panel[., 2] = TT :* detval[., 2]

    _spmixw_sem(y, X, W, N, TT, ndraw, nomit, thin, mod_,
                rval, nu, d0, c_beta_col, C_beta_mat,
                detval_panel, rmin, rmax,
                bdraw, sdraw, pdraw, vmean, beta_pm, sige_pm, rho_pm)

    // Joint (beta, lambda) draws so the coefficient table can show λ as We.
...
// ----------------------------------------------------------------------
// Display helper — same Bayesian table format as SAR/OLS, with row label
// "We" for the spatial-error parameter λ.
// ----------------------------------------------------------------------
program _spmixw_sem_display
    version 13.0
    ...
    di
    di as txt "Bayesian SEM panel (spmixw)" _col(58) "N obs    = " ///
        as res %9.0f e(N)
    di as txt "Model:    sem" _col(58) "N groups = " ///
        as res %9.0f e(N_g)
